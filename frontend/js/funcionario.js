...
function carregarRegistrosMes(id) {
    ...
    const mes = parseInt(sessionStorage.getItem("dash_mes") || (new Date().getMonth() + 1));
    const ano = parseInt(sessionStorage.getItem("dash_ano") || new Date().getFullYear());
...
function calcularEExibir(registros, mes, ano) {
    ...
    if (elSaldo) {
        if (saldoMin === 0)     { elSaldo.textContent = "00:00"; }
        else if (saldoMin > 0)  { elSaldo.textContent = "+" + fmtMin(saldoMin); elSaldo.classList.add("verde"); }
        else                    { elSaldo.textContent = "-" + fmtMin(Math.abs(saldoMin)); elSaldo.classList.add("vermelho"); }
    }
    if (elFaltas) {
...
function validarHorarios(e1, s1, e2, s2, e3, s3, evento) {
    ...
    if (turnoInvalido(e1, s1)) erros.push("Turno 1: entrada depois da saída. Se cruzou meia-noite, está correto.");
